...
RobotWallet::RobotWallet(Robot* entity_) : WalletDecorator(entity) {
  ...
  // calculate trip cost
  entity->SetTripDistance(entity->GetPosition().Distance(
      entity->GetDestination()));  // set trip distance
  tripCost = static_cast<int>(ceil(0.1 * entity->GetTripDistance()));
...
void RobotWallet::Update(double dt, std::vector<IEntity*> scheduler) {
  ...
  if (start) {
    if ((money >= tripCost)) {
      ...
      std::string strat =
          entity->GetStrategyName();  // set toBank to that location
      if (strat == "astar")
        ...
        toBank =
            new DijkstraStrategy(entity->GetPosition(), nearestBank, graph);
      ...
      tripCost = static_cast<int>(ceil(0.1 * entity->GetTripDistance()));
      std::cout << "From the bank, the robot will now pay $" << tripCost
                << " for this trip." << std::endl;
