...
int DataCollection::calcPopularBank() {
  ...
  for (auto& i : bankVisits) {
    if (i.second >= greatest) {
      // if the bank has more visits than the last, replace the holder and move
      // to the next index
      greatest = i.second;
      returnVal++;
    }
...
void DataCollection::writeToCSV() {
  ...
  myfile << "Enitity, Total Distance, Total Money Collected, Total Delivery "
            "Trips, Total Bank Trips, Total Trips, Bank Visits, Total Money "
            "Deposited, Trip Cost, Distance per Trip, Money per Trip, Money "
            "Per Deposit, Most Popular Bank \n";
  ...
  if (totalDistTrav.size() == 0) {
    ...
    tDist = totalDistTrav.at(droneName);
  }
  if (totalMoney.size() == 0) {
    tMon = 0;
  } else {
    tMon = totalMoney.at(droneNameWal);
  }
  if (totalDelTrips.size() == 0) {
    tDTrips = 0;
  } else {
    tDTrips = totalDelTrips.at(droneNameDelTrip);
  }
  if (totalBankTrips.size() == 0) {
    tBTrips = 0;
  } else {
    tBTrips = totalBankTrips.at(droneNameBankTrip);
  }
  if (moneyDeposited.size() == 0) {
    tMDep = 0;
  } else {
    tMDep = moneyDeposited.at(droneNameDepos);
  }
  ...
  // drone data
  myfile << "Drone," << tDist << "," << tMon << "," << tDTrips << "," << tBTrips
         << "," << totalTrips << ", N/A," << tMDep << ", N/A,"
         << calcDelDistPerTrip(droneName) << "," << calcMoneyPerTrip() << ","
         << calcMoneyPerDeposit() << "," << calcPopularBank() << "\n";

  // bank data
  int bankCount = 1;
  for (auto& k : bankVisits) {
    // only data for banks currently is how many times they have been visited
    myfile << "Bank " << bankCount << ","
           << "N/A,"
           << "N/A,"
           << "N/A,"
           << "N/A,"
